...
void startBuffer(char* program, char* input, char mode, short size, int increment) {
	...
	/* create buffer */
	bufferp = bufferCreate(size, (char)increment, mode);
	...
	loadSize = bufferLoad(bufferp, fileHandler);
	 ...
	 */
	if (loadSize != BUFFER_ERROR) {
		if (!bufferAddChar(bufferp, BUFFER_EOF)) {
